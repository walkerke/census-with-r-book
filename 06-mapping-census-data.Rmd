...
To begin, let's grab some ACS data on race and ethnicity from the 2016-2020 American Community Survey. We'll be looking at data on non-Hispanic white, non-Hispanic Black, Asian, and Hispanic populations for Census tracts in Hennepin County, Minnesota.
...
In this example, we specify `value = "estimate"` to visualize the estimated counts from the ACS; `values_per_dot = 100` for a data:dots ratio of 100 people per dot; and `group = "variables"` to partition dots by racial / ethnic group on the map.
