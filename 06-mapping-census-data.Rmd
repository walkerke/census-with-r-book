...
tmap's map-making syntax will be somewhat familiar to users of ggplot2, as it uses the concept of _layers_ to specify modifications to the map.  The map object is initialized with `tm_shape()`, which then allows us to view the Census tracts with `tm_polygons()`.  
...
## Merging tabular data to Census geographic data 
...
## Visualizing thousands of Census shapes with mapdeck
...
tx_bachelors_cleaned <- tx_bachelors %>%
  filter(!st_is_empty(.))

mapdeck(style = mapdeck_style("light")) %>%
  add_polygon(data = tx_bachelors_cleaned,
