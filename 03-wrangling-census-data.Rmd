...
style_data(mn_hh_income, caption = "Table B19001 for counties in Minnesota, 2012-2016 ACS")
```
...
style_data(oglala_lakota_age, caption = "2016-2020 age table for Oglala Lakota County, SD")
```
...
```{r oglala-lakota-2010, error = TRUE}
...
style_data(oglala_lakota_age_10, caption = "200-2010 age table for Oglala Lakota County, SD (then named Shannon County)")
```
...
**New content: comparison profile tables**

The safest option for time-series analysis in the ACS is to use the Comparison Profile Tables. These tables are available for both the
