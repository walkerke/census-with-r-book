...
Chapter \@ref(4) introduced the concept of *faceted plots* to compare different views, which is also a very useful concept for geographic visualization. Faceted mapping will be addressed directly in the next chapter. For comparative spatial plots, the [patchwork R package](https://patchwork.data-imaginist.com/) @pedersen2020 works very well for arranging a multi-plot layout. Below, we'll use patchwork to put two ggplot2 spatial plots - one of Census tracts and one of block groups in Los Alamos County - side-by-side using the `+` operator.
...
```{r la_tracts_mapview, echo = FALSE}
...
In addition to the core TIGER/Line shapefiles, after which the tigris package is named, the Census Bureau also makes available *cartographic boundary shapefiles*. These files are derived from the TIGER/Line shapefiles but are generalized in the interior and clipped to the shoreline of the United States, making them a better choice in many cases than the TIGER/Line shapefiles for thematic mapping. Most polygonal datasets in tigris are available as cartographic boundary files, accessible with the argument `cb = TRUE`.

Over the years I have been developing tigris, I have gotten multiple inquires akin to "why does this area look so strange when I download it?" Michigan, with its extensive shoreline along the Great Lakes, commonly comes up. The TIGER/Line shapefiles include water area for geographic features, connecting the Upper Peninsula of Michigan with the southern part of the state and giving an unfamiliar representation of Michigan's land area. Using the cartographic boundary alternative resolves this. Let's use patchwork to compare the TIGER/Line and cartographic boundary shapefiles for counties in Michigan to illustrate this.
...
While the TIGER/Line shapefiles may represent "official" areas of counties - which include water area - they look very unfamiliar to viewers expecting a usual representation of land area in Michigan. Alternatively, the cartographic boundary file shows islands and a distinct coastline, and is the better alternative for thematic mapping. When using the `cb = TRUE` argument, users can also specify one of three resolutions with the `resolution` argument: `"500k"` (the default), `"5m"`, or `"20m"`, with higher values representing more generalized boundaries and smaller file sizes.
...
The cache directory can be checked with the `user_cache_dir()` function available in the rappdirs package. The specific location of your cache directory will depend on your operating system.
...
Tarrant County added 180 new Census tracts between 1990 and 2020. As the plot shows, many of these tracts are found in fast-growing parts of the county in the northeast and southeast.
...
That said, a common request from tigris users is to obtain *all shapes* for the United States. For example, a national demographic study may require all 220,000 block groups in the United States. While this option is not directly available in tigris, national datasets are straightforward to create with tidyverse tools. As covered in several examples in this book to this point, the `purrr::map()` family of functions iterate over a sequence of values and combine function results as directed by a user. For tigris users, the `map_dfr()` function will prove especially useful as it row-binds datasets to create its output. The built-in `state.abb` vector in R gives us postal codes of all 50 US states; if Washington, DC or Puerto Rico are required in your analysis, add these to the vector with `c()` as shown below.

Once the vector of state codes is prepared, a user can iterate over those codes in a single line of code and produce a national block group dataset.

```{r all-block-groups}

state_codes <- c(state.abb, "DC", "PR")

us_bgs <- map_dfr(state_codes, ~block_groups(state = .x, cb = TRUE))

glimpse(us_bgs)
```

If you are not using shapefile caching, this process may take a while; however, if you have a local cache of block group shapefiles as illustrated in this section, loading and combining all 220,000 block groups for use in your analysis should only take a few seconds.

## Coordinate reference systems

For geographic data to appropriately represent locations in mapping and spatial analysis, they must be *referenced* to some model of the Earth's surface correctly. In simpler terms - a data model of the state of Florida should represent where Florida is actually located relative to other locations! This is accomplished with *coordinate reference systems*, or CRSs, which define not only how data coordinates should be mapped to a model of the Earth's surface, but also how measurements should be computed using a given dataset. A more complete discussion of CRSs is found in [@lovelace_geocomputation_2019]; an overview of how to work with coordinate systems in relationship to tigris is covered below.
...
While this representation puts all territories in their appropriate locations, it is clearly not appropriate for Alaska, Hawaii, and Puerto Rico which appear distorted. Additionally, any comparative mapping of the United States using this coordinate reference system would suffer from substantial whitespace (covered in more detail in the next chapter).
