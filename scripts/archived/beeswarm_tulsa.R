...
df <- get_acs(geography = "tract", state = "OK",   
...
metro <- core_based_statistical_areas(cb = TRUE, class = "sf") %>%
  filter(str_detect(NAME, "Tulsa"))

tulsa <- df[metro, op = st_within]

ggplot(tulsa, aes(x = variable, y = summary_est, color = summary_est)) +
  ...
  labs(x = "Largest group in Census tract", 
       ...
       subtitle = "Census tracts, Tulsa metropolitan area", 
