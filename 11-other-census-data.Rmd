...
```{r generate-italy-dots, eval = FALSE}
...
```{r generate-italy-dots2, echo = FALSE}
italy_dots <- read_rds("data/italy_dots.rds")
...
blscrapeR includes a number of helper functions to assist users with some of the most commonly requested statistics. For example, the `get_bls_county()` function fetches employment data for all counties in the US for the latest month available without any arguments.
...
style_data(latest_employment, caption = "latest_employment")
```
...
```{r series_ids, eval = FALSE}
head(series_ids)
```

```{r show-series_ids, echo = FALSE}
style_data(series_ids, n_rows = 6, caption = "series_ids")
```
...
We'll use this information to get unadjusted data on accommodation workers in Maui, Hawaii since the beginning of 2018, and make a chart with the data. Note that the specific codes detailed above will vary from indicator to indicator, and that not all industries will be available for all geographies.
...
style_data(maui_accom, 6, "maui_hotel")
```
...
hud_chas_request %>%
  content(as = "text") %>%
  fromJSON() %>%
  style_data(n_rows = 1, caption = "hud_chas_request")
```
