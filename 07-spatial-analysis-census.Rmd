...
```{r gilbert-compare, echo = FALSE, fig.cap = "Comparison of Census tracts in Gilbert, AZ from the 2010 and 2020 Census"}
library(patchwork)
...
```{r gilbert-pop-weights, echo = FALSE, fig.cap="Illustration of block points and population weights"}
...
# First map: block points
gb1 <- ggplot() + 
  gilbert_basemap + 
  geom_sf(data = gilbert_2020, color = "red", fill = NA) + 
  geom_sf(data = gilbert_blocks, color = "navy") + 
  theme_void() + 
  labs(title = "Illustration of block points")
...
gb2 <- ggplot() + 
  ...
  labs(title = "Illustration of population weights") + 
  theme_void()
...
```{r catchment-results, echo = FALSE}
catchment_tibble <- tibble(
  Method = c("5km buffer", "10min isochrone"),
  `Families in poverty` = c(buffer_pov$poverty_numE,
                            iso_pov$poverty_numE),
  `Total families` = c(buffer_pov$poverty_denomE,
                       iso_pov$poverty_denomE),
  Percent = c(round(buffer_pov$pct_poverty, 1),
              round(iso_pov$pct_poverty, 1))
)

style_data(catchment_tibble, caption = "Comparison of buffer and isochrone catchment areas")
```
